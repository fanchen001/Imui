...
import com.fanchen.popup.enums.LayoutStatus;
...
public class SmartDragLayout extends FrameLayout implements NestedScrollingParent {
    ...
    public void smoothScroll(final int dy, final boolean isOpen) {
        post(new Runnable() {
            @Override
            public void run() {
                scroller.startScroll(getScrollX(), getScrollY(), 0, dy, (int)(isOpen ? Popup.getAnimationDuration() : Popup.getAnimationDuration()*0.8f));
