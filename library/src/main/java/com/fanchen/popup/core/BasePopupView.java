...
import com.fanchen.popup.animator.BlurAnimator;
import com.fanchen.popup.animator.EmptyAnimator;
import com.fanchen.popup.animator.PopupAnimator;
import com.fanchen.popup.animator.ScaleAlphaAnimator;
import com.fanchen.popup.animator.ScrollScaleAnimator;
import com.fanchen.popup.animator.ShadowBgAnimator;
import com.fanchen.popup.animator.TranslateAlphaAnimator;
import com.fanchen.popup.animator.TranslateAnimator;
import com.fanchen.popup.enums.PopupStatus;
import com.fanchen.popup.impl.FullScreenPopupView;
import com.fanchen.popup.impl.PartShadowPopupView;
import com.fanchen.popup.util.KeyboardUtils;
import com.fanchen.popup.util.PopupUtils;
import com.fanchen.popup.util.navbar.NavigationBarObserver;
import com.fanchen.popup.util.navbar.OnNavigationBarListener;
import java.util.ArrayList;
import java.util.List;
import java.util.Stack;
import static com.fanchen.popup.enums.PopupAnimation.NoAnimation;
...
public abstract class BasePopupView extends FrameLayout implements OnNavigationBarListener, LifecycleObserver {
    ...
    private void collectAnimator() {
        if (popupContentAnimator == null) {
            ...
            if(popupInfo.hasBlurBg) {
                ...
                blurAnimator.decorBitmap = PopupUtils.view2Bitmap(((Activity)getContext()).getWindow().getDecorView());
    ...
    protected void applySize(boolean isShowNavBar) {
        //获取屏幕高度
        int height = PopupUtils.getPhoneScreenHeight(dialog.getWindow());
        //获取应用内屏幕可用高度
        ...
        boolean isNavBarShown = PopupUtils.isNavBarVisible(dialog.getWindow());
        //设置margin为屏幕高度减去应用可用高度
        if (rotation == 0) {
            params.leftMargin = 0;
            params.rightMargin = 0;
//            params.bottomMargin = isNavBarShown ? PopupUtils.getPhoneScreenHeight(getContext()) - height : 0;
            params.bottomMargin = 0;
        } else if (rotation == 1) {
            params.bottomMargin = 0;
            params.rightMargin = isNavBarShown ? PopupUtils.getPhoneScreenHeight(dialog.getWindow()) - height : 0;
            params.leftMargin = 0;
        } else if (rotation == 3) {
            params.bottomMargin = 0;
            params.leftMargin = 0;
    ...
    public int getAnimationDuration() {
        return popupInfo.popupAnimation == NoAnimation ? 10 : Popup.getAnimationDuration()+10;
    }
    ...
    public void smartDismiss() {
        handler.post(new Runnable() {
            @Override
            public void run() {
                delayDismiss(Popup.getAnimationDuration()+50);
            }
        });
    }
