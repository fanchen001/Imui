...
public class SwipeMenuLayout extends ViewGroup {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //左滑右滑的开关,默认左滑打开菜单
        isLeftSwipe = true;
        TypedArray ta = context.getTheme().obtainStyledAttributes(attrs, R.styleable.MSwipeMenuLayout, defStyleAttr, 0);
        int count = ta.getIndexCount();
        for (int i = 0; i < count; i++) {
            int attr = ta.getIndex(i);
            //如果引用成AndroidLib 资源都不是常量，无法使用switch case
            if (attr == R.styleable.MSwipeMenuLayout_swipeEnable) {
                isSwipeEnable = ta.getBoolean(attr, true);
            } else if (attr == R.styleable.MSwipeMenuLayout_ios) {
                isIos = ta.getBoolean(attr, false);
            } else if (attr == R.styleable.MSwipeMenuLayout_leftSwipe) {
        ...
        if (isSwipeEnable) {
            ...
            switch (ev.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (Math.abs(gap) > 20 || Math.abs(getScrollX()) > 20) {//2016 09 29 修改此处，使屏蔽父布局滑动更加灵敏，
                        getParent().requestDisallowInterceptTouchEvent(true);
                        isMoved = true;
                    }
...
/*                    if (!mScroller.isFinished()) {
                    ...
                    if(isMoved)scrollBy((int) (gap), 0);//滑动使用scrollBy
                    //越界修正
                    if (isLeftSwipe && isMoved) {//左滑
                        if (getScrollX() < 0) {
                            scrollTo(0, 0);
                        }
                    ...
                    if(isMoved)mLastP.set(ev.getRawX(), ev.getRawY());
                    break;
