...
public class PermissionActivity extends AppCompatActivity {
    ...
    private void onDeny(String permission, int position) {
        if (mCallback != null && mCallback instanceof PermissionCallback2)
            ((PermissionCallback2)mCallback).onDeny(permission, position);
    }

    private void onGuarantee(String permission, int position) {
        if (mCallback != null && mCallback instanceof PermissionCallback2)
            ((PermissionCallback2)mCallback).onGuarantee(permission, position);
    }
