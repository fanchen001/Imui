package com.fanchen.crash.compat;
...
//          IBinder binder = clientTransaction.getActivityToken();
            Class<?> name = Class.forName("android.app.servertransaction.ClientTransaction");
            Method getActivityToken = name.getDeclaredMethod("getActivityToken");
            getActivityToken.setAccessible(true);
            IBinder binder = (IBinder) getActivityToken.invoke(message.obj);
            finish(binder);
        }catch (Throwable e){
            e.printStackTrace();
        }
